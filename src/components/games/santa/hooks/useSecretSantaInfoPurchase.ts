...
export const useSecretSantaInfoPurchase = (listKey?: string, onSuccess?: () => void) => {
  ...
  const handleAct = async (payload: Purchase) => {
    try {
      ...
      onSuccess?.()
      setNotify('Услуга успешно оплачена, вы всегда сможете увидеть, кто является Вашим Сантой', {
        severity: 'success',
      })
    } catch (error) {
  ...
  const getPopupMessages = () => {
    return {
      ...
      message: 'Вы сможете увидеть того, кто является Вашим Сантой. Оплату нельзя будет отменить!',
    }
  }
